const SC_NS = "http://songbirdnest.com/data/1.0#";
const SC_streamName = SC_NS + "streamName";
const SC_bitRate = SC_NS + "bitRate";			// SBProperties.bitRate
const SC_comment = SC_NS + "comment";			// SBProperties.comment
const SC_mimeType = SC_NS + "contentMimeType";	// SBProperties.contentMimeType
const SC_listenerCount = SC_NS + "listenerCount";
const SC_id = SC_NS + "streamId";
const SC_bookmark = SC_NS + "streamFave";
const SC_unfave = SC_NS + "streamUnfave";

const SC_nodeCreated = SC_NS + "favesSPSNodeCreated";
