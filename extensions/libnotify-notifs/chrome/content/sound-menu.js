...
if (typeof NotifProxy == 'undefined') {
	var NotifProxy = {};
};

NotifProxy.soundMenu = {
	xulAppInfo: null,
	...
	onLoad: function () {
		...
		this.prefs = Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.libnotify-notifs.");
		this.prefs.QueryInterface(Components.interfaces.nsIPrefBranch2);
		
		NotifProxy.soundMenu.preferencesObserver.register();
		
		this.unityServiceProxy = Components.classes["@getnightingale.com/Songbird-Nightingale/UnityProxy;1"].getService(Components.interfaces.IUnityProxy);
		...
		NotifProxy.soundMenu.preferencesObserver.observe(null, "needInit", "enableNotifications");
		NotifProxy.soundMenu.preferencesObserver.observe(null, "needInit", "hideOnClose");
	...
	onUnLoad: function () {
		NotifProxy.soundMenu.preferencesObserver.unregister();
	},
	...
	preferencesObserver: {
		register: function () {
			NotifProxy.soundMenu.prefs.addObserver("", this, false);
		},
		
		unregister: function () {
			NotifProxy.soundMenu.prefs.removeObserver("", this);
		},
		...
		observe: function (aSubject, aTopic, aData) {
			...
			switch (aData) {					
				case "hideOnClose":
					NotifProxy.soundMenu.registerOnClose(true);
					break;
					
				case "enableNotifications":
					var enableNotifPref = NotifProxy.soundMenu.prefs.getBoolPref("enableNotifications");
					var enableNotifTBtn = document.getElementById("libnotify-notifs-enableNotifTButton");
					if (enableNotifPref) {
						enableNotifTBtn.checked = false;
						enableNotifTBtn.setAttribute("tooltiptext", "Notifications enabled");
						enableNotifTBtn.image = "chrome://libnotify-notifs/content/notif-enabled.png";
					}
					else {
						enableNotifTBtn.checked = true;
						enableNotifTBtn.setAttribute("tooltiptext", "Notifications disabled");
						enableNotifTBtn.image = "chrome://libnotify-notifs/content/notif-disabled.png";
					}
					NotifProxy.soundMenu.unityServiceProxy.EnableNotifications(enableNotifPref);
			}
		}
	}
};
...
function onNotifTButtonClick (element) {
	NotifProxy.soundMenu.prefs.setBoolPref("enableNotifications", !element.checked);
}

window.addEventListener("load",   function(e) { NotifProxy.soundMenu.onLoad(); },   false);
window.addEventListener("unload", function(e) { NotifProxy.soundMenu.onUnLoad(); }, false);
