...
/*
			// Condensed text discography
			var prevYear = "999999999";
			var table = doc.createElement("table");
			table.className = "discography";
			var list;
			var prevLi = null;
			results.forEach(function(val, i, arr) {
				var year;
				if (val.release_date != null)
					year = val.release_date.substr(0,4);
				else
					year = "";
				...
				if (year != prevYear) {
					...
					// Set the last list item we added to not show a ,
					if (prevLi != null)
						prevLi.className = "last";

					// Create a new row for the new year
					var row = doc.createElement("tr");
					var yearCol = doc.createElement("td");
					yearCol.className = "year";
					yearCol.appendChild(doc.createTextNode(year));
					row.appendChild(yearCol);

					var albumCol = doc.createElement("td");
					albumCol.className = "albums";
					list = doc.createElement("ul");
					list.className = "flat";
					albumCol.appendChild(list);
				...
				if (val.link != null) {
					album = doc.createElement("a");
					album.href = val.link;
					album.appendChild(doc.createTextNode(val.title));
				} else {
					album = doc.createTextNode(val.title);
				}
				li.appendChild(album);
				if (Application.prefs.getValue(
						"extensions.mashTape.discography.albumart", false) &&
						val.artwork != null)
				{
					var img = doc.createElement("img");
					img.src = val.artwork;
					img.className = "artwork tipper";
					img.setAttribute("title", "DOM:mbid-img-" + val.mbid);

					var divArtwork = doc.createElement("div");
					divArtwork.id = "mbid-img-" + val.mbid;
					divArtwork.innerHTML = "<img src='" + val.artwork + "'>";
					divArtwork.style.display = "none";
					discoDiv.appendChild(divArtwork);
			...
			if (prevLi != null)
				prevLi.className = "last";
			discoDiv.appendChild(table);
	
			var scriptDiv = doc.getElementById("scripty");
			var script = doc.createElement('script');
			var codeNode = doc.createTextNode(
