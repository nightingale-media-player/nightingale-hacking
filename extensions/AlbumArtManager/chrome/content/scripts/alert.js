...
function prefillAlertInfo()
{
  ...
  var alertText = window.arguments[2];
  var alertTextArray = [alertText];
  var alertTextLabelBox = document.getElementById('alertTextLabelBox');
  
  if (alertText.indexOf('\n') > 0) {
    alertTextArray = alertText.split('\n');
  }
  
  alertTextArray.forEach( function(mElement) {
    var alertTextLabel = document.createElement('label');
    alertTextLabel.setAttribute('value', mElement);
    if (gAlertTextClickable) {
      alertTextLabel.setAttribute('clickable', true);
    }
    alertTextLabel.setAttribute('class', "alertText");
    alertTextLabelBox.appendChild(alertTextLabel);
  });
