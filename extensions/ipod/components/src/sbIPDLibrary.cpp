...
NS_IMETHODIMP
sbIPDLibrary::SetSyncPlaylistList(nsIArray* aPlaylistList)
{
  // Validate arguments.
  ...
  // Forward to the base device library.
  rv = sbDeviceLibrary::SetSyncPlaylistList(aPlaylistList);
  NS_ENSURE_SUCCESS(rv, rv);
...
sbIPDLibrary::AddToSyncPlaylistList(sbIMediaList* aPlaylist)
{
  ...
  // Forward to the base device library.
  rv = sbDeviceLibrary::AddToSyncPlaylistList(aPlaylist);
  NS_ENSURE_SUCCESS(rv, rv);
...
sbIPDLibrary::GetMgmtType(PRUint32* aMgmtType)
{
  ...
  // Forward to the base device library.
  rv = sbDeviceLibrary::GetMgmtType(aMgmtType);
  NS_ENSURE_SUCCESS(rv, rv);
...
sbIPDLibrary::SetMgmtType(PRUint32 aMgmtType)
{
  ...
  // Forward to the base device library.
  rv = sbDeviceLibrary::SetMgmtType(aMgmtType);
  NS_ENSURE_SUCCESS(rv, rv);
...
sbIPDLibrary::InitializePrefs()
{
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  rv = SetSyncPlaylistListPref(syncPlaylistList);
  NS_ENSURE_SUCCESS(rv, rv);
