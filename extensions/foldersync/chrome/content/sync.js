...
foldersync.sync={
    ...
    getProfile: {
      ...
      run: function(){
        try {
          ...
          this.crt = foldersync.JSON.parse(foldersync.JSON.stringify(this.crt));
          
          // Get default tags and append them (include to the profile)
          var defTags = foldersync.preferences.getDefaultTags();
          defTags = foldersync.JSON.parse(foldersync.JSON.stringify(defTags));
          ...
          fallbacks = foldersync.JSON.parse(foldersync.JSON.stringify(fallbacks));
    ...
    sync: {
      ...
      run: function(){
        try {
          ...
          this._running = false;
          try{this._notify({event:"fatal"}, {sync:{}});
          foldersync.central.logEvent("sync-sync",
                                    "Sync thread terminated " +
                                    "unexpected:\n\n" + e, 1,
                                    "chrome://foldersync/content/sync.js",
                                    e.lineNumber);}catch(e2){foldersync.central.logEvent("sync-sync",e2,1);}
        }
      },
    },
  },
  ...
  getQueue: function(){
    ...
    return foldersync.JSON.parse(foldersync.JSON.stringify(this._queue));
  },
};