...
function pfServicePane_servicePaneInit(sps) {
  ...
  var mainWindow = wm.getMostRecentWindow("Nightingale:Main");
  if (mainWindow)
    this._playlistfolders = mainWindow.playlistfolders;
  else{
    Cc['@mozilla.org/observer-service;1'].getService(Ci.nsIObserverService).
      addObserver(this, 'nightingale-main-window-presented', false);
  }
}
...
function pfServicePane_fillNewItemMenu(aNode, aContextMenu, aParentWindow) {
  ...
  var stringBundle = sbSvc.createBundle("chrome://nightingale/locale/nightingale." +
                                        "properties");
...
function pfServicePane_fillFolderMenu(aNode, aContextMenu, aParentWindow) {
  ...
  menuitem.setAttribute("oncommand", "gServicePane.startEditingNode(this.node);");
	menuitem.node = aNode;
  aContextMenu.appendChild(menuitem);
}
...
function pfServicePane_onDragGesture(aNode, aTransferable) {
  // Ignore non-folders
  if (aNode.id.split(URN_PREFIX_FOLDER).length <= 1)
    return false;
	aTransferable.addDataFlavor(TYPE_X_PF_TRANSFER_FOLDER);
	var text = Components.classes["@mozilla.org/supports-string;1"].
     createInstance(Components.interfaces.nsISupportsString);
  text.data = aNode.id;
  aTransferable.setTransferData(TYPE_X_PF_TRANSFER_FOLDER, text,
                                text.data.length*2);
  return true;
}
...
function pfServicePane_observe(subject, topic, data) {
  // Link 
  if (topic == 'nightingale-main-window-presented') {
    // connect to the global controllers of this AddOn
    var wm = Components.classes["@mozilla.org/appshell/window-mediator;1"].
                        getService(Components.interfaces.nsIWindowMediator);
    var mainWindow = wm.getMostRecentWindow("Nightingale:Main");
    ...
    Cc['@mozilla.org/observer-service;1'].getService(Ci.nsIObserverService).
      removeObserver(this, 'nightingale-main-window-presented');
  }
}
