...
/** 
* \file  Thsource.cpp
* \brief Songbird Thsource Component Implementation.
*/
...
#include "nsRDFBuiltInThsources.h"
*/
...
#ifdef PR_LOGGING
/*
 * NSPR_LOG_MODULES=sbThsource:5
 */
static PRLogModuleInfo* gThsourceLog = PR_NewLogModule("sbThsource");
#endif
#define LOG(args) PR_LOG(gThsourceLog, PR_LOG_DEBUG, (args))

static  CThsource  *gThsource = nsnull;
...
NS_IMETHODIMP MyQueryCallback::OnQueryEnd(sbIDatabaseResult *dbResultObject, const PRUnichar *dbGUID, const PRUnichar *strQuery)
{
  ...
  m_Timer->InitWithFuncCallback( &MyTimerCallbackFunc, gThsource, 0, 0 );
...
{
  nsString( NS_LITERAL_STRING("&thsource.welcome") ),
  nsString( NS_LITERAL_STRING("&thsource.library") ),
  nsString( NS_LITERAL_STRING("&thsource.bookmarks") ),
  nsString( NS_LITERAL_STRING("&thsource.searches") ),
  nsString( NS_LITERAL_STRING("&thsource.music_stores") ),
  nsString( NS_LITERAL_STRING("&thsource.podcasts") ),
  nsString( NS_LITERAL_STRING("&thsource.radio") ),
...
// CLASSES ====================================================================
NS_IMPL_ISUPPORTS2(CThsource, sbIThsource, nsIRDFDataSource)
//-----------------------------------------------------------------------------
CThsource::CThsource()
{
  Init();
...
//-----------------------------------------------------------------------------
/*virtual*/ CThsource::~CThsource()
{
  DeInit();

} //dtor

void CThsource::Update(void)
{
...
void CThsource::DeInit (void)
{
  VMETHOD_SHORTCIRCUIT;
#ifdef DEBUG_REFS
  --gInstanceCount;
  fprintf(stdout, "%d - RDF: CThsource\n", gInstanceCount);
#endif
  {
    if ( nsnull != gRDFService )
    {

      NS_RELEASE(kNC_Thsource);
      NS_RELEASE(kNC_ThsourceFlat);
...
NS_IMETHODIMP
CThsource::GetSource(nsIRDFResource* property,
                    nsIRDFNode* target,
                    PRBool tv,
...
NS_IMETHODIMP
CThsource::GetSources(nsIRDFResource *property,
                     nsIRDFNode *target,
                     PRBool tv,
...
NS_IMETHODIMP
CThsource::GetTarget(nsIRDFResource *source,
...
NS_IMETHODIMP
CThsource::GetTargets(nsIRDFResource *source,
                     nsIRDFResource *property,
                     PRBool tv,
                     nsISimpleEnumerator **targets /* out */)
{
  ...
  *targets = nsnull;
  ...
  else if (source == kNC_ThsourceFlat)
  {
    if (property == kNC_child)
    {
      nsresult rv = NS_OK;
...
NS_IMETHODIMP
CThsource::Assert(nsIRDFResource *source,
                 nsIRDFResource *property,
                 nsIRDFNode *target,
                 PRBool tv)
{
...
NS_IMETHODIMP
CThsource::Unassert(nsIRDFResource *source,
                   nsIRDFResource *property,
                   nsIRDFNode *target)
{
  METHOD_SHORTCIRCUIT;
...
NS_IMETHODIMP
CThsource::Change(nsIRDFResource* aSource,
...
NS_IMETHODIMP
CThsource::Move(nsIRDFResource* aOldSource,
...
NS_IMETHODIMP
CThsource::HasAssertion(nsIRDFResource *source,
                       nsIRDFResource *property,
                       nsIRDFNode *target,
                       PRBool tv,
...
NS_IMETHODIMP 
CThsource::HasArcIn(nsIRDFNode *aNode, nsIRDFResource *aArc, PRBool *result)
{
  METHOD_SHORTCIRCUIT;
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
NS_IMETHODIMP 
CThsource::HasArcOut(nsIRDFResource *aSource, nsIRDFResource *aArc, PRBool *result)
{
  METHOD_SHORTCIRCUIT;
  *result = PR_FALSE;

  if (aSource == kNC_Thsource)
  {
...
NS_IMETHODIMP
CThsource::ArcLabelsIn(nsIRDFNode *node,
...
NS_IMETHODIMP
CThsource::ArcLabelsOut(nsIRDFResource *source,
                       nsISimpleEnumerator **labels /* out */)
{
  METHOD_SHORTCIRCUIT;
  NS_PRECONDITION(source != nsnull, "null ptr");
...
NS_IMETHODIMP
CThsource::GetAllResources(nsISimpleEnumerator** aCursor)
{
  METHOD_SHORTCIRCUIT;
...
NS_IMETHODIMP
CThsource::AddObserver(nsIRDFObserver *n)
{
  METHOD_SHORTCIRCUIT;
...
NS_IMETHODIMP
CThsource::RemoveObserver(nsIRDFObserver *n)
{
  METHOD_SHORTCIRCUIT;
  NS_PRECONDITION(n != nsnull, "null ptr");
  if (! n)
...
NS_IMETHODIMP
CThsource::GetAllCmds(nsIRDFResource* source,
...
NS_IMETHODIMP
CThsource::IsCommandEnabled(nsISupportsArray/*<nsIRDFResource>*/* aSources,
...
NS_IMETHODIMP
CThsource::DoCommand(nsISupportsArray/*<nsIRDFResource>*/* aSources,
                    nsIRDFResource*   aCommand,
                    nsISupportsArray/*<nsIRDFResource>*/* aArguments)
{
  METHOD_SHORTCIRCUIT;
