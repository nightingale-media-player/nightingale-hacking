...
function PublicPlaylistCommands() {
  this.m_mgr = Components.
    classes["@getnightingale.com/Nightingale/PlaylistCommandsManager;1"]
...
PublicPlaylistCommands.prototype = {
  ...
  classDescription: "Nightingale Public Playlist Commands",
  classID:          Components.ID("{1126ee77-2d85-4f79-a07a-b014da404e53}"),
  contractID:       "@getnightingale.com/Nightingale/PublicPlaylistCommands;1",
  ...
  initCommands: function() {
    try {
      ...
      const PlaylistCommandsBuilder = new Components.
        Constructor("@getnightingale.com/Nightingale/PlaylistCommandsBuilder;1",
     ...
     this.m_cmd_UpdateSmartPlaylist.setCommandVisibleCallback(null, 
      ...
      var webListGUID =
        prefs.getComplexValue("nightingale.library.web",
...
function plCmd_LookupCDInfo_TriggerCallback(aContext, aSubMenuId, aCommandId, aHost) {
  ...
  var devMgr = Cc["@getnightingale.com/Nightingale/DeviceManager;2"]
...
function playlist_setRipProperty(aPlaylist, aVal, aNewVal) {
  ...
  mediaList.enumerateItemsByProperty(
                   "http://getnightingale.com/data/1.0#shouldRip",
                   aVal,
                   {
...
function plCmd_RenameList_TriggerCallback(aContext, aSubMenuId, aCommandId, aHost) {
  ...
  if (servicePane) {
    var librarySPS = Cc['@getnightingale.com/servicepane/library;1']
...
function plCmd_IsRescanItemEnabled(aContext, aSubMenuId, aCommandId, aHost) {
  ...
  var enabled = false;
  try {
   enabled = prefs.getBoolPref("nightingale.commands.enableRescanItem");
  } catch (e) {
    // nothing to do
  }
  return enabled;
...
function getDownloadDevice() {
  ...
  {
    if (!g_downloadDevice) {
      var deviceManager = Cc["@getnightingale.com/Nightingale/DeviceManager;1"]
                            .getService(Ci.sbIDeviceManager);

      if (deviceManager)
      {
        var downloadCategory = 'Nightingale Download Device';
...
function getWebLibrary() {
  try {
    if (g_webLibrary == null) {
      var prefs = Cc["@mozilla.org/preferences-service;1"]
        .getService(Ci.nsIPrefBranch2);
      var webListGUID = prefs.getComplexValue("nightingale.library.web",
          Ci.nsISupportsString);
      var libraryManager =
...
function dataRemote(aKey, aRoot) {
  if (!g_dataRemoteService) {
    g_dataRemoteService = new Components.
      Constructor( "@getnightingale.com/Nightingale/DataRemote;1",
                  Ci.sbIDataRemote,
                  "init");
  }
