...
function sbNightingaleDispatch() {}
sbNightingaleDispatch.prototype = {
  classDescription: "Nightingale nightingale:// protocol command dispatch",
  classID:          Components.ID("2c15f30e-815f-4414-8d18-277d943a2f68"),
  contractID:       "@mozilla.org/network/protocol;1?name=nightingale",
 
  newChannel: function(aUri) {
    if (!(aUri.scheme == "nightingale")) {
    ...
    // nightingale://open/?url=http://foo.com
    ...
    if (aUri.host) {
      switch(aUri.host) {
          ...
          try {
            ...
            if (aUri.scheme == "chrome") {
              Components.utils.reportError(
               "sbNightingaleProtocol::newChannel: " +
            ...
            Components.utils.reportError(
               ...
               "\nThe syntax is: nightingale:open?url=<encodedURL>");
            return new BogusChannel(aUri, "application/dummy-mime-type");
          }
        ...
        case "nocommand":
          ...
          Components.utils.reportError(
                  "sbNightingaleProtocol::newChannel: no command: " + origURISpec);
          return new BogusChannel(aUri, "application/dummy-mime-type");
          break;
        default:
          Components.utils.reportError(
                  "sbNightingaleProtocol::newChannel: bad command: " + aUri.host);
          return new BogusChannel(aUri, "application/dummy-mime-type");
          break;
      }
    } else {
      Components.utils.reportError(
              "sbNightingaleProtocol::newChannel: no command: " + aUri.spec);
      return new BogusChannel(aUri, "application/dummy-mime-type");
    }
  },
  ...
  newURI: function (spec, charSet, baseURI) {
    ...
    if (!url.host) {
      ...
      // mozbug 478478
      Components.utils.reportError("sbNightingaleProtocol::newURI: no command provided.\n"+
                                   "Syntax is in the form: nightingale:<command>?p1=a&p2=b");
...
function NSGetModule(compMgr, fileSpec) {
  return XPCOMUtils.generateModule([sbNightingaleDispatch]);
}
