...
sbGStreamerMediacore::InitPreferences()
{
  ...
  rv = mPrefs->AddObserver("nightingale.mediacore", this, PR_FALSE);
  NS_ENSURE_SUCCESS(rv, rv);
...
sbGStreamerMediacore::ReadPreferences()
{
  ...
  rv = mPrefs->GetBoolPref("nightingale.mediacore.gstreamer.disablevideo", 
	                       &mVideoDisabled);
  ...
  PRInt32 prefType;
  const char *VIDEO_SINK_PREF = "nightingale.mediacore.gstreamer.videosink";
  const char *AUDIO_SINK_PREF = "nightingale.mediacore.gstreamer.audiosink";
  ...
  const char *AUDIO_SINK_BUFFERTIME_PREF = 
      "nightingale.mediacore.output.buffertime";
  /* In kilobytes */
  const char *STREAMING_BUFFERSIZE_PREF = 
      "nightingale.mediacore.streaming.buffersize";
  ...
  const char *NORMALIZATION_ENABLED_PREF = 
      "nightingale.mediacore.normalization.enabled";
  const char *NORMALIZATION_MODE_PREF = 
      "nightingale.mediacore.normalization.preferredGain";
...
PRBool sbGStreamerMediacore::HandleSynchronousMessage(GstMessage *aMessage)
{
  ...
  switch (msg_type) {
    case GST_MESSAGE_ELEMENT: {
      // Win32 and GDK use prepare-xwindow-id. Mac is handled asynchronously.
...
void sbGStreamerMediacore::HandleMessage (GstMessage *message)
{
  ...
  switch (msg_type) {
    ...
    case GST_MESSAGE_ELEMENT: {
      ...
      else if (gst_structure_has_name(message->structure, "have-ns-view"))
      {
          DispatchMediacoreEvent(sbIMediacoreEvent::STREAM_HAS_VIDEO);
          mPlatformInterface->PrepareVideoWindow(message);
      }
      break;
    }
    default:
...
sbGStreamerMediacore::OnShutdown()
{
  ...
  if (mPrefs) {
    nsresult rv = mPrefs->RemoveObserver("nightingale.mediacore.gstreamer", this);
    NS_ENSURE_SUCCESS(rv, rv);
  }
