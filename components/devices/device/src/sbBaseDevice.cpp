...
#include <sbILibraryServicePaneService.h>
                                      ...
                                      sbIMediaList**    aSyncMediaList)
{
  ...
  nsCOMPtr<sbILibraryServicePaneService> LSPS =
    do_ProxiedGetService("@songbirdnest.com/servicepane/library;1", &rv);
  NS_ENSURE_SUCCESS(rv, rv);

  nsString uniqueName;
  rv = LSPS->SuggestUniqueNameForPlaylist(aSrcLib, listName, uniqueName);
  NS_ENSURE_SUCCESS(rv, rv);
