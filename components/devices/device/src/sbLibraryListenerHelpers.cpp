...
sbPlaylistSyncListener::sbPlaylistSyncListener(sbILibrary* aDeviceLibrary)
: mDeviceLibrary(aDeviceLibrary)
{
  NS_ASSERTION(aDeviceLibrary,
               "sbPlaylistSyncListener cannot be given a null aDeviceLibrary");
}
                                    ...
                                    PRBool *_retval)
{
  NS_ENSURE_ARG_POINTER(aMediaList);
  NS_ENSURE_ARG_POINTER(aMediaItem);
  NS_ENSURE_TRUE(mDeviceLibrary, NS_ERROR_NOT_INITIALIZED);
                                           ...
                                           PRBool *_retval)
{
  ...
  nsCOMPtr<sbIMediaItem> deviceListAsItem;
  rv = sbLibraryUtils::GetItemInLibrary(aMediaList, 
                                        mDeviceLibrary,
                                        getter_AddRefs(deviceListAsItem));
  NS_ENSURE_SUCCESS(rv, rv);

  nsCOMPtr<sbIMediaList> deviceMediaList = 
    do_QueryInterface(deviceListAsItem, &rv);
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  NS_ENSURE_SUCCESS(rv, rv);
  PRBool debugEquals;
  NS_ASSERTION(NS_SUCCEEDED(debugItem->Equals(deviceListAsItem, 
