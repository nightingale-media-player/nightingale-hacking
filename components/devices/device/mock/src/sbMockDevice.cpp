...
#define DEVICE_PREF_BRANCH \
  "nightingale.devices.mock.00000000-0000-0000-c000-000000000046."
...
NS_IMETHODIMP sbMockDevice::GetProperties(sbIDeviceProperties * *theProperties)
{
  ...
  if(!mProperties) {
    nsCOMPtr<sbIDeviceProperties> properties = 
      do_CreateInstance("@getnightingale.com/Nightingale/Device/DeviceProperties;1", &rv);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv = writeBag->SetProperty(NS_LITERAL_STRING("http://getnightingale.com/device/1.0#freeSpace"), 
                               freeSpace);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv = writeBag->SetProperty(NS_LITERAL_STRING("http://getnightingale.com/device/1.0#totalUsedSpace"),
                               totalUsedSpace);
    NS_ENSURE_SUCCESS(rv, rv);
