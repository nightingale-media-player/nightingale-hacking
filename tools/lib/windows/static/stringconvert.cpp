...
tstring ConvertUTF16toUTFn(const std::wstring& src) {
  #if defined(XP_WIN) && defined(_UNICODE)
    return src;
  #else
...
tstring ConvertToUTFn(LPCSTR src)
{
  return ConvertUTF8toUTFn(src);
}

tstring ConvertToUTFn(LPCWSTR src)
{
  return ConvertUTF16toUTFn(src);
}
