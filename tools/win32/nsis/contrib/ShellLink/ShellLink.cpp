...
char szBuf2[MAX_STRLEN]="";
int nBuf;
WORD wHotkey;

void ShortCutData(int nType, HWND hwndParent, int string_size, char *variables, stack_t **stacktop);
...
void ShortCutData(int nType, HWND hwndParent, int string_size, char *variables, stack_t **stacktop)
{
  ...
  g_variables=variables;
  {
				...
				if (nType <= SHELLLINKTYPE_GETWORKINGDIR)
				{
					...
					switch(nType)
					{
						...
						case SHELLLINKTYPE_GETHOTKEY: 
						{
							...
							if (hRes == S_OK) wsprintf(szBuf, "%d", wHotkey);
							else szBuf[0]='\0';
						}; break;
						...
						{
							hRes=m_psl->GetIconLocation(szBuf, MAX_PATH, &nBuf);
							if (hRes == S_OK) wsprintf(szBuf, "%d", nBuf);
							else szBuf[0]='\0';
						}; break;
	...
	{
		if (nType <= SHELLLINKTYPE_GETWORKINGDIR) pushstring("");
		else pushstring("-1");
	}
  }
}
