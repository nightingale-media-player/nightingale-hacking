...
{  
  ...
  {    
    ...
    @files_list = split(/\n/,
       `find $source_path/* -type f ! -path "*.svn*" ! -name ".*" ! -name "Makefile.in" ! -name "jar.mn" ! -name "jar.mn.in" ! -name "*.vcproj" ! -name "_EXCLUDE" ! -name "*.user"  ! -name "Thumbs.db" ! -name "*.rej" ! -name "*.orig" ! -name "*.ncb" ! -name "*.diff" ! -name "*.patch"`);
