...
{  
  ...
  {    
    ...
    @files_list = split(/\n/,
       `find $source_path/* -name ".svn*" -a -prune -o -type f ! -name ".*" ! -name "Makefile.in" ! -name "jar.mn" ! -name "jar.mn.in" ! -name "*.vcproj" ! -name "_EXCLUDE" ! -name "*.user"  ! -name "Thumbs.db" ! -name "*.rej" ! -name "*.orig" ! -name "*.ncb" ! -name "*.diff" ! -name "*.patch" ! -name "*.vpj" -a -print`);
