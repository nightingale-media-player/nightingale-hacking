...
function Engine(aLocation, aSourceDataType, aIsReadOnly) {
  ...
  if (aLocation instanceof Ci.nsILocalFile) {
    ...
    switch (aLocation.scheme) {
      ...
      case "file":
      case "resource":
      case "chrome": // XXX Nightingale: allow chrome search engines
        this._uri = aLocation;
        break;
      default:
...
Engine.prototype = {
  ...
  _type: null,
  // XXX Nightingale: Space delimited keyword string
  _tags: "",
  ...
  _setIcon: function SRCH_ENG_setIcon(aIconURL, aIsPreferred) {
    ...
    switch (uri.scheme) {
      // XXX Nightingale: Accept chrome icons. Needed for library stub.
      case "chrome":
      case "data":
        this._iconURI = uri;
  ...
  _parseAsOpenSearch: function SRCH_ENG_parseAsOS() {
    ...
    for (var i = 0; i < doc.childNodes.length; ++i) {
      ...
      switch (child.localName) {
        ...
        // XXX Nightingale: extensions for the internal search
        case "Alias":                                                           
...
SearchService.prototype = {
  ...
  _addEngineToStore: function SRCH_SVC_addEngineToStore(aEngine) {
    ...
    // XXX Nightingale HACK
    // For now any engines with special nightingale tags should be hidden by default
    ...
    if (aEngine.tags.indexOf("nightingale") > -1) {
      aEngine.hidden = true;
    }
